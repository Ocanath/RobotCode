...
void init_forward_kinematics(DH_entry * DH_Table, HTmatrix * HTadj, HTmatrix * HTbase, int size)
{
	...
	{
		//[row][column]
		HTadj[i].H[0][0] = cos(DH_Table[i].theta);		HTadj[i].H[0][1] = -sin(DH_Table[i].theta)*cos(DH_Table[i].alpha);		HTadj[i].H[0][2] = sin(DH_Table[i].theta)*sin(DH_Table[i].alpha); 	HTadj[i].H[0][3] = DH_Table[i].a*cos(DH_Table[i].theta);
		HTadj[i].H[1][0] = sin(DH_Table[i].theta);		HTadj[i].H[1][1] = cos(DH_Table[i].theta)*cos(DH_Table[i].alpha);		HTadj[i].H[1][2] = -cos(DH_Table[i].theta)*sin(DH_Table[i].alpha);	HTadj[i].H[1][3] = DH_Table[i].a*sin(DH_Table[i].theta);
...
//
//INPUTS: c-style pass by reference jacobian and transpose arrays, and the chain size
...
OUTPUTS: J_Transpose
*/
void JacobianTranspose(double J[6][AVICS_NUM_DOFS], double J_Transpose[AVICS_NUM_DOFS][6], int size)
{
	int r, c;
	for (r = 0; r < size; r++)
	{
...
//	z = (Vect3 *)malloc(size * sizeof(Vect3));
    Vect3 d[AVICS_ARR_SIZE];
    Vect3 z[AVICS_NUM_DOFS];
