    ...
    {       
        ...
        while(!DataRdyUART2())
        {
            ...
            for (i = 1; i <= 6; i++)
            {
                ...
                Twitchy_Leg_Forward_Kinematics(TW_theta[i],
                    TLG0_1[i], TLG1_2[i], TLG2_3[i],
                    TW_alpha1, TW_alpha2, TW_alpha3, TW_a1, TW_a2, TW_a3,
                    TLG0_2[i], TLG0_3[i]);

                double TWITCHY_J[6][3]; //six rows five cols
                double TRANSPOS_TWITCHY_J[3][6];
                double TXV, TYV, TZV, TXR, TYR, TZR;
                Twitchy_Leg_Inverse_Kinematics(T_xpos[i], T_ypos[i], T_zpos[i], T_xOrient[i], T_yOrient[i], T_zOrient[i],
                    TW_theta[i], TW_prev_theta[i], TLG0_1[i], TLG0_2[i], TLG0_3[i],
                    TWITCHY_J, TRANSPOS_TWITCHY_J,
                    &TXV, &TYV, &TZV, &TXR, &TYR, &TZR, .000015);		//for mouse, 2 orders of magnitude smaller!!!! i.e. divide by 100 at least

                int j;
                for (j = 1; j <= 3; j++)
                {
                    TW_theta[i][j] += TRANSPOS_TWITCHY_J[j - 1][0] * TXV + TRANSPOS_TWITCHY_J[j - 1][1] * TYV + TRANSPOS_TWITCHY_J[j - 1][2] * TZV;
                    TW_prev_theta[i][j] = TW_theta[i][j];
                }
                Twitchy_Leg_Limit_Joints(TW_theta[i]);
            }
